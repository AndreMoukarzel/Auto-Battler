...
func buff(unit, effect, ally_army, enemy_army):
	...
	var attr_affected: String = effect[1][0].to_lower()
	var modifier: String = effect[1][1].to_lower()
	var value: int = effect[1][2][unit.level - 1]
	...
	if target_team.to_lower() == "ally":
		...
		if target_pos != -1 and ally_army.units[target_pos] != null:
			...
			modify_attribute(target_unit, attr_affected, modifier, value)
	else:
		push_warning("[EffectHandler][Buff] UNKOWN TARGET ARMY TYPE " + target_team)


func damage(unit, effect, ally_army, enemy_army):
	var damage_value: int = effect[1][unit.level - 1]
	var target_team: String = effect[2][0] # Ally | Enemy
	var target_direction: String = effect[2][1]
	var frequency: int = effect[3][unit.level - 1]
...
func modify_attribute(target_unit, attr_affected: String, modifier: String, value: int):
	if attr_affected == "attack":
		if modifier == "sum":
			target_unit.add_bonus_attack(value)
		elif modifier == "mult":
			target_unit.add_bonus_attack(int(target_unit.curr_attack * (value - 1.0)))
	elif attr_affected == "health":
		if modifier == "sum":
			target_unit.add_bonus_health(value)
		elif modifier == "mult":
			target_unit.add_bonus_health(int(target_unit.curr_health * (value - 1.0)))
